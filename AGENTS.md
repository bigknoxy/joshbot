...
- **The shell allowlist screens the whole command, not the first word, and refuses chaining.** `checkAllowList` (`internal/tools/shell.go`) rejects any command containing `;`, `&`, `|`, a newline, a backtick, `$(`, `<(` or `>(` while an allowlist is in force, because the command reaches `sh -c` unchanged and `echo hi; id` passed a first-word match. It does not exempt quoted occurrences on purpose — a boundary that depends on a quoting parser matching the shell's exactly is not a boundary, and `ls | wc -l` being refused costs a caller one extra call. `defaultUnsandboxedAllowlist` also no longer contains `find`, `go` or `git`: each launches a program of the caller's choosing (`find -exec sh -c`, `go run`, `git -c core.pager=...`). Both `Execute` and `ExecuteAsync` go through the one helper — adding a third entry point means calling it too.
- **`EvalSymlinks` cannot tell a dangling symlink from a missing file**, so `resolveSymlinks` (`internal/tools/path_guard.go`) must resolve every re-appended component, not just the nearest existing ancestor: `ws/evil -> /outside/x` with no `/outside/x` returned the lexical path, passed `isWithinBase`, and was then written. Resolution narrows but does not close the TOCTOU window either — the final component can still be swapped between the check and the open — which is why filesystem writes go through `writeNoFollow` (`O_NOFOLLOW`, `internal/tools/filesystem.go`). A new write path needs the same treatment.
