...
- **A one-shot cron job's countdown restarts with the process**: `delay:` is a duration from *scheduling time*, and nothing records an absolute due moment, so a persisted `delay:30m` fires 30 minutes after the next start. One-shot jobs retire themselves once they fire (`retireJob`) — without that they replay on every boot forever. Recurring jobs run until deleted.
