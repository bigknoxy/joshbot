...
- **The fallback notice rides the reply text, and on the streaming path the buffer and the reply must stay identical**: `providers.WithFallbackNotice` (`internal/providers/notice.go`) carries a per-request callback the way `agent.WithSink` does; `MultiProvider` fires it only when a provider other than the addressed one answers, with the addressed provider's error class as the reason (`"cooldown"` when it was never dialled). The agent prepends `formatFallbackNotice` to the reply rather than sending it out of band, so every channel carries it with no per-channel wiring; `agents.defaults.quiet_fallback` (bool, deliberately inverted so the zero value shows notices — a default-true bool would need a schema migration, the streaming v4→v5 trap) suppresses it. Two rules on the streaming path (`streamChat`): the notice is emitted **lazily, right before the first content delta** — eagerly it would surface for a tool-call-only response that shows no text — and it is prepended to the returned response content whenever it was shown, because a delta the sink displayed that the reply text lacks (or vice versa) breaks the Telegram streamer's `shown == buf` suppression rule and re-sends the whole reply through the bus fallback.
