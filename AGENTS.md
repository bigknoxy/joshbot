...
- **Agent progress callback is optional and nil by default**: `agent.Agent` has an unexported `progress ProgressFunc` field, set via `agent.WithProgressCallback` (an `Option`, for construction time) or `Agent.SetProgressCallback` (a method, for wiring/unwiring after construction). Nil means the ReAct loop in `internal/agent/agent.go` never calls it — zero behavior change for callers that don't opt in (Telegram, non-interactive `agent -m`, tests). `cmd/joshbot/main.go`'s `runAgentLoop` is the only current caller of `SetProgressCallback`, and only when `isTTY(output)` is true.
