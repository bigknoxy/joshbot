...
- **`providers.<name>.timeout` is only consumed for ollama today** (`registerProviders`, `cmd/joshbot/main.go`). Every other provider silently ignores the key and takes `providers.Config`'s default. Do not document it as universal.
