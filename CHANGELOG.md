...
### ⚠️ Breaking
- **Telegram: an empty `allow_from` now denies every sender** (previously it allowed everyone). A bot with no allowlist handed anyone on the internet a direct line into an agent loop holding the shell tool. `IsAllowed` now fails closed on an empty allowlist and `Start` logs a loud warning naming the exact key to set (`channels.telegram.allow_from` / `JOSHBOT_CHANNELS__TELEGRAM__ALLOW_FROM`). **If you relied on an open bot, add your numeric Telegram user ID to `allow_from` or it will reject all messages.** The new Discord channel enforces the same fail-closed rule.

### Added
- **Agent-friendly CLI parity** (#144, #148, #149) — `agent` gained `--output-format text|json|stream-json` (JSON modes are non-interactive and require `-m`; stdout carries only the result document, logs go to stderr; `cost_usd` is `null` with token `usage` provided), `--resume`/`--session` to thread a prior session by id, and a stable **exit-code contract** (`0` ok, `1` general, `2` auth/no-provider, `3` validation, `4` confirmation-reserved) with machine-readable JSON errors on stderr. New global `--no-color` and `--log-level debug|info|warn|error` flags (the latter takes precedence over `--verbose`/`--debug`).
- **Non-interactive onboarding** (#142) — `onboard` gained `--provider`, `--api-key` and `--api-base`; the API key also falls back to `JOSHBOT_PROVIDERS__<PROVIDER>__API_KEY`. After saving, the credential is validated (non-fatal) and the provider's key URL is printed if it looks wrong.
- **Configurable heartbeat interval + completion contract** (#141) — new `heartbeat.interval` config key (Go duration, default 30m, `JOSHBOT_HEARTBEAT__INTERVAL` override, reconciling the old hardcoded 5m with the documented 30m). Each task is now published with a marker telling the agent it is an automated background check and to reply exactly `HEARTBEAT_OK` when nothing needs attention; those (and empty) replies are suppressed. A tick is skipped entirely (tasks left unchecked to retry) when no recipient chat ID is known, and a task is only checked off once actually published, so it never re-fires or burns tokens against a dead end.
- **Discord channel** — a second chat channel (gateway websocket + REST via the pure-Go `discordgo`, compiled into the single binary). Deny-by-default `allow_from` (numeric IDs / usernames / global names), 2000-char code-fence-aware splitting, typing keep-alive, `/help` and `/new`, ignores bot/self messages. New `channels.discord` config (`enabled`/`token`/`allow_from`) with `JOSHBOT_CHANNELS__DISCORD__ENABLED`/`__TOKEN` overrides.
- **macOS shell sandbox** (#150) — a real Seatbelt (`sandbox-exec`) profile mirroring the Linux Landlock design, so `tools.shell_sandbox: "workspace"` now enforces filesystem/network containment on macOS as well as Linux. On platforms with no sandbox available, the shell tool falls back to **allowlist-only** by default (non-escaping read/inspect commands) unless the operator sets an explicit `shell_allow_list`. The runtime default remains `"off"`.
- **stdio MCP client** (#155) — `internal/mcp` implements a lazy-start, clean-shutdown JSON-RPC 2.0 MCP client; `mcp` config declares servers (`command`/`args`/`env`/`enabled`). Discovered tools register under a namespaced `mcp__<server>__<tool>` so they can never shadow a built-in. Config is the trust boundary (out-of-workspace, operator-only). *Not yet wired into the running agent — declaring a server does not spawn a process today; MCP children are not sandboxed.*
- **Regression protection** (#154, #156) — CLI golden-output tests, per-package coverage floors in CI (`cmd/joshbot` ≥ 15%, `internal/service` ≥ 2%), and an opt-in `-tags liveeval` prompt-behaviour eval against a live provider.
