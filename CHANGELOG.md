...
- **MCP client** (#155) — stdio-transport Model Context Protocol servers configurable in `config.json`; discovered tools register into the normal tool registry, namespaced `mcp__<server>__<tool>` so an MCP server cannot shadow a built-in tool. Not yet wired into `cmd/joshbot/main.go`, so no server is spawned by a release build.
