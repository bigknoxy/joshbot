...
- CI: automated security scanning (`govulncheck`, `gosec`, `CodeQL`, `gitleaks`) runs on
  ...
  Branch protection on `main` now requires CI plus the three blocking security jobs.
...
- **`channels.telegram.reactions`: reactions-as-acknowledgement (#314).** Puts an emoji on the user's *own* message — 👀 the moment the turn is admitted to the bus, 👍 when the reply is on its way — so a phone user gets an "it heard me" signal before the first token of the reply exists, at the cost of no message slot (which is what makes it usable in groups). Three things shape it. The completion emoji is 👍 and **not** ✅: ✅ is not in Telegram's free reaction set, so it is rejected with `REACTION_INVALID` and the acknowledgement silently never appears — a test pins both emoji against the documented free list. `setMessageReaction` (Bot API 7.0) **sets** rather than appends, so writing 👍 clears 👀 with no separate clearing call. And it is **opt-in, default off**, carrying `omitempty` for the same reason `api.webui` does — a `Config` bool without it is written into every saved config and its default can then never be flipped without a schema migration (the `streaming` v4→v5 trap) — because a bot in a group without permission to react would otherwise log a failure on every single turn. A reaction is an ornament on the turn and never part of it: a failure is logged at debug and the reply is unaffected, and a sender the allowlist rejects is never acknowledged. This is **outbound only**; joshbot does not subscribe to `message_reaction` updates.
- **Telegram command menus are scoped to the allowlist (#316).** joshbot now publishes a `BotCommandScopeChat` menu for each numeric `channels.telegram.allow_from` entry and **deletes** the global all-private-chats menu, so a stranger who finds the bot sees no command menu at all rather than a full list of things they are not allowed to run. Username-shaped allowlist entries cannot be resolved to a chat id until that user speaks, so if any exist the global menu is kept — an operator allowlisted by name would otherwise be left with nothing. An empty allowlist denies every sender, so it deletes the menu too. Clearing a scope must use **`deleteMyCommands`, not an empty `setMyCommands`**: telebot's `CommandParams.Commands` carries `json:"commands,omitempty"`, so an empty slice omits the field entirely and the Bot API rejects the call — `DeleteCommands` was added to the channel's notifier interface for exactly this. A per-chat registration for an allowlisted user who has never started the bot fails with "chat not found", which is expected and is logged and skipped: one such user must not cost every other user their menu. The first error is still returned, and registration failure remains non-fatal at startup. This completes **command names only** — Telegram has no mechanism to complete paths or arguments, so it does not close the gap with the TUI's Tab completion.

### Fixed

- **`onboard --force` honours a provider API key in the environment, and a failed onboard no longer poisons the next (found dogfooding).** `JOSHBOT_PROVIDERS__NVIDIA__API_KEY=... joshbot onboard --force` failed with an error telling the operator to set exactly the variable they had set: with no `--provider` flag the force path hardwired openrouter and only checked *that* provider's env key. Worse, a failed run writes the defaults config — a disabled, keyless openrouter seed — and the next `--force` picked its provider from that config (via a nondeterministic map range), shadowing the env credential forever after. Provider precedence is now: `--provider` flag → a *usable* provider from the existing config (one holding a key, or keyless-by-design ollama/github-copilot when enabled, checked in menu order so the pick is deterministic) → the first supported provider with a key in the environment → openrouter. `./scripts/verify-local.sh` completes with only an env key present, which is how the bug was found.

- **The TUI's Tab completion no longer offers commands that do nothing.** `cliCommandNames` listed `/clear` and `/history` for a buffered prompt that never implemented them, so Tab completed a command the agent then answered as ordinary prose — a silent failure with no error anywhere. Both are gone, `/resume` (which the agent has always handled) was added, and `/exit` now matches alongside the bare `exit` the pre-editor prompt accepted. A new test parses `handleCommand`'s switch out of `internal/agent/agent.go` with `go/ast` and fails if the completion list ever drifts from the dispatch again, rather than duplicating the list by hand.

- **`channels.telegram.stream_drafts`: native streaming drafts and the "Thinking…" placeholder (#308, #309).** Streams a turn through the Bot API `sendMessageDraft` method instead of the `sendMessage` + `editMessageText` loop, so Telegram renders the growing answer as a native animated draft; before any output exists an empty-text draft renders as Telegram's own "Thinking…" placeholder, so a phone shows the turn started before the first token arrives. Off by default and carrying `omitempty`, so it is absent from every config joshbot has already saved and needs no schema migration — the trap the `streaming` bool hit at v4→v5. **The version in the issue title is wrong and worth recording**: the published Bot API reference dates the empty-text allowance to **Bot API 10.0 (8 May 2026)**, not the "9.3/9.5" #308 claims. telebot v3.3.8 has no typed wrapper for the method, so the call goes out through `Bot.Raw`, which already turns an `{"ok":false}` body into an error — that is what makes the self-disable reliable rather than a silent no-op, and it honours `channels.telegram.api_url` for free. Three constraints define the feature. It is **private-chat only**, because the reference documents `chat_id` as the target private chat, so a group would spend a request per delta to earn an error per delta. The first refusal from the API disables drafts **for that turn only** and the edit loop carries the answer from that same delta on, never failing the reply — streaming is presentation, and losing the answer to it would be a poor trade. And a draft is **ephemeral** (the reference calls it a temporary 30-second preview and requires an ordinary `sendMessage` to persist the text), so a draft write never sets `delivered` and `TelegramStreamer.Finish`'s `delivered && !broken && shown == buf` suppression contract is untouched — the persisting send still runs at the end of every turn. Tool progress rides the draft slot too, so in draft mode no status message is created and `Finish` has none to clean up. The 4096-character tail is cut on a **rune boundary**, because a byte slice through a multi-byte rune is invalid UTF-8 and Telegram rejects the whole request for it.
...
- **Telegram inline-button presses are routed structurally instead of being re-typed as user text (#312).** `handleCallback` used to synthesize `[Callback: <data>]` as a *user message* and push it through the bus, so every button press spent a full ReAct turn -- an LLM round trip, tool budget and session append -- to interpret a payload the code had already produced. `internal/channels/callback.go` adds an envelope, `namespace:action:payload`, split with `SplitN(..., 3)` so only the first two fields are structural and a payload may itself contain `:`. `TelegramChannel.RegisterCallback(namespace, handler)` claims a namespace (refusing an empty one, one containing the separator, a nil handler, or a duplicate registration) and a decoded press is delivered to it as a typed `CallbackPress` carrying the action, callback id, chat id, message id and sender. Data that does not decode still falls through to the old `[Callback: %s]` bus inbound, so existing keyboards keep working. Two details are load-bearing: `CallbackAction.Encode` **errors at build time** when the envelope exceeds Telegram's 64-byte `callback_data` cap rather than truncating -- a truncated payload decodes as a *different, valid* action -- and returns `""` on failure so a caller that ignores the error cannot send a half-encoded button; and `DecodeCallback` trims a leading `\f`, which telebot prepends to the data of a unique-endpoint button.
- **Both inline-keyboard branches of `buildReplyMarkup` were dead code.** They type-asserted `markup["inline_keyboard"].([][]map[string]any)` -- a concrete Go slice type that JSON-decoded metadata can never satisfy, since `encoding/json` produces `[]any` of `[]any` of `map[string]any`, and no in-process Go producer existed either. Every keyboard silently became an empty markup with no error anywhere. `normalizeKeyboardRows` now accepts both shapes, and a `*channels.Keyboard` may be passed through `Metadata["reply_markup"]` directly for in-process callers.
- **`/new` is no longer stuck behind the turn it is meant to escape, and a reset cannot be undone by an in-flight turn (#319).** Every turn is serialised per session key (#236), and `/new` was serialised with them — so a reset issued during a long tool-running turn waited for that turn, and under the per-key cap (`MaxConcurrentTurnsPerKey`) it was not merely slow but refused outright with the "too many turns in flight" backpressure reply. That is the opposite of what a reset is for. `/new` is now routed **ahead of** `session.Manager.LockSession` in `agent.Process`, so it answers immediately. Skipping the lock is only safe with a fence, so sessions carry a `generation`: `Manager.ResetConversation` bumps it and `Manager.Save` refuses any write whose generation is older than the one on disk, which stops the turn that loaded its prefix before the reset from republishing the transcript the user just cleared. A refused write is logged at debug and the turn still reports success — a dropped stale write is the fence working, not a failure the user should see. **The filed issue's premise was wrong and the fix is not what it asked for**: the lock already covered `/new`, and Telegram's `handleNew` does no local clearing (it publishes the command to the bus), so the two paths could not drift. The real defect was that the reset was not privileged. En route, the archive-based `Manager.Reset` behind `joshbot sessions reset` was found to have the same exposure from the other side: it deleted the metadata sidecar outright, which erased the fence, so it now rewrites the sidecar carrying the bumped generation alone. `SessionResetter` is an optional interface checked by type assertion, like `SessionLocker`, so a store that does not implement it keeps the previous in-lock behaviour. The `generation` field carries `omitempty` and is zero for every session that has never been reset, so no schema migration is needed.

- **The onboard wizard validates the Telegram token against `channels.telegram.api_url` and no longer erases keys it does not collect (#321).** `channels.ValidateToken` hardcoded `api.telegram.org`, so on a LAN-only self-hosted `telegram-bot-api` setup — a primary #280 use case — the wizard reported a perfectly working token as invalid. `ValidateTokenAt(token, apiURL)` dials the configured endpoint, with empty meaning the public API, exactly as the message path treats the key. Fixing that surfaced the sibling bug: `setupTelegram` returned a struct built from only the three fields it collects and `runOnboard` assigns it wholesale, so every onboard re-run silently erased `api_url`, `reactions` and `stream_drafts` from a saved config. Every wizard return now derives from the existing `TelegramConfig` and changes only what the wizard actually asked about.
- **A tag with no matching CHANGELOG section now fails the release, not every subsequent PR (#293).** `changelog-guard` runs on pull requests, so `v1.57.0` tagged with its entries still under `[Unreleased]` shipped fine and then failed the guard for every later contributor, with a message pointing at their changelog rather than at the tag. The release workflow now gates everything behind `cmd/relguard -require-section` (`relguard.CheckTagHasSection`), which refuses a tag with no `## [x.y.z]` heading, naming the tag and the missing heading. The PR-side guard also distinguishes the two failure shapes: when the *base branch's own* changelog lacks the newest tag's section, it says "this is a release problem on the base branch, not a problem with this PR" instead of sending an innocent contributor hunting through their diff.
- **Streaming turns no longer report zero token usage (#301).** The API's usage plumbing was correct end to end, but nothing ever asked the upstream for usage on a stream: OpenAI-compatible endpoints only emit a usage-bearing final chunk when the request carries `stream_options: {"include_usage": true}`, and the SSE reader dropped choice-less chunks — which is exactly what the usage frame is — before anything could read it. Streaming requests now send the field, the reader forwards the usage frame, and `ChunkAccumulator` captures it even though it arrives *after* the finish-reason chunk (the one chunk that legitimately does), so `Response.Usage` reaches the existing `agent.WithUsageSink` wiring and both the HTTP API's final SSE frame and `agent -m --json` report real counts. Zeros were worse than a missing field — they read as a measurement. Endpoints that reject `stream_options` can opt out per provider (`providers.<name>.disable_stream_usage: true`) or per model in the model-centric format; the zero value sends it, so the key carries `omitempty` and needs no schema migration.

### Changed

- **Telegram formatting now goes on the wire as HTML (#315).** Replies are still authored and split as Markdown, but every outbound path -- message text (`internal/channels/telegram.go`), attachment captions (`telegram_attachment.go`) and the streamer's final edit (`telegram_stream.go`) -- converts to HTML at the point the payload is handed to Telegram. The legacy Markdown parser rejects ordinary LLM prose (`snake_case`, a stray `*`, an unclosed backtick) with `400 can't parse entities`, and the existing recovery then re-sends the whole answer with **every** bit of formatting stripped; HTML needs exactly three escapes (`&`, `<`, `>`) instead of MarkdownV2's eighteen, so the same prose survives. The converter (`MarkdownToHTML`, `internal/channels/telegram_html.go`) escapes at interpolation, treats an unterminated marker as literal text, never parses inside code spans or fences, and allowlists link schemes to `http`/`https`/`tg`. The plain-text fallback still sends the **pre-conversion** Markdown source, so a rejected message never reaches the reader as a literal `&amp;`. Splitting still happens on the Markdown with the fence-aware splitter, so no tag straddles a part boundary. Callers are unaffected: `parse_mode: "markdown"` remains the metadata contract.
## [1.59.0] - 2026-08-19

### Added

- **Outbound media on Telegram: the `send_file` tool (#277).** joshbot can now deliver a file from its workspace to the chat as a real attachment — a photo renders inline, anything else arrives as a document — instead of describing a path the user cannot open. It is a **separate tool**, not a flag on `message`: a caption-less send is legitimate here and `message` rejects empty content, and a distinct name is what lets the model be told the containment rules that apply to bytes leaving the process. **Containment is the boundary and it is enforced twice:** the path goes through the same `resolvePath` workspace check and the same `openat(2)` component walk (`internal/tools/openat.go`) the `filesystem` tool uses, so a lexical escape and an intermediate-symlink escape both error and send nothing — verified by mutation to be genuinely independent layers, either one alone still refuses. **Content decides the type, never the extension**: the first 512 bytes are sniffed, so a text file named `.png` is sent as a document and a JPEG named `.dat` is sent as a photo, the same rule inbound images already follow. Every file is carried as bytes on the message, read once through the handle the contained walk opened — there is deliberately no second route, so nothing downstream re-opens a path. Both limits are 10 MiB, below Telegram's own 50 MiB document ceiling, because the payload is held in memory and the cap is therefore a memory bound. The transport is a typed `bus.Attachment` slice on `OutboundMessage` rather than a `Metadata` key — a typo in a map sends nothing and reports success. On Telegram a failed send is retried once through the existing `isRetryable` classification with the payload **rebuilt each attempt** (a consumed reader would upload zero bytes), a `can't parse entities` failure is retried with the caption's parse mode cleared and only the caption's, and a caption over Telegram's 1,024-character limit is sent as its own following message rather than truncated. Limits are reachable through `AttachmentLimits()` rather than hardcoded at each use site, so #280 can raise them from a local Bot API server. The recipient is **not an argument**: `internal/agent` puts the inbound turn's channel on the request context and the tool reads it back, so the model cannot address a file anywhere — a turn with no channel is refused, never defaulted. Channels with no attachment support (Discord today) **degrade honestly**: the workspace-relative path, name and size are appended to the message text under an explicit `[attachment not supported on this channel]` marker, never silently dropped.

- **Inbound PDF documents on Telegram (#278).** A PDF sent in a Telegram chat is now downloaded (after the allowlist check, never before) and carried on the turn as a document attachment, so the model reads the file instead of guessing from `[Document: report.pdf]`. It rides the same rail images do: `providers.Document` on `Message.Documents`, serialized at the single OpenAI-compatible point as a `file` content part (`{"type":"file","file":{"filename":…,"file_data":"data:application/pdf;base64,…"}}`), so every provider is covered by one `MarshalJSON` and a text-only message still serializes byte-identically. **Document capability is a separate, narrower list than vision** — a model that reads images does not necessarily parse PDFs — and `screenForDocuments` runs it against every candidate model before the first network call from **both** `Chat` and `ChatStream`, failing closed on unknown models with an error naming the models tried and the config key to change, rather than a provider 400 mid-conversation. **Content decides the type:** the bytes must start with `%PDF-`, so a PNG named `report.pdf` is refused and a PDF named `shot.png` is not smuggled through the image path; the declared MIME and size decide only whether a download is worth spending. Limits are 8 MiB per document and 16 MiB per request, refused from the declared size before any transfer and then read through a `LimitReader` at cap+1 so a file that lies about its size is refused rather than silently truncated into a corrupt PDF. Sessions record a `DocumentRef` — label, type, size, SHA-256 — and never the bytes. A PDF that cannot be read ends the turn with an honest error in the chat. Office formats (docx, xlsx, pptx) remain unsupported, and the refusal now lists what *is* supported: PDFs, text files and images.

## [1.58.0] - 2026-08-18

### Added

- **Browser chat UI behind `api.webui` (#201).** `joshbot serve` can now serve a self-contained chat page at `/`: streaming replies over the same `POST /v1/chat/completions` every other client uses, a transcript that survives a reload via a new read-only `GET /webui/session`, and a "New conversation" button that mints a fresh browser-side session key rather than deleting anything. The page, its stylesheet and its script are embedded with `//go:embed` and load nothing from the network — no CDN, system fonts only — so the single-binary, zero-runtime-deps property holds. **It is off by default and that default is a security decision, not a rollout preference:** the page is a login form that accepts an `api.api_keys` value, and that key reaches the shell and filesystem tools, so a form like it must not appear on every existing `serve` bind at upgrade; with `api.webui` false every web UI route answers **404 as if the feature were absent** and a session cookie is not honoured. When on, `serve` prints the URL in its banner. **Authentication:** a browser cannot ship a bearer key against a fail-closed server, `?key=` is refused (it leaks into history, proxy logs and `Referer`), and a loopback exemption is refused (it disables auth for every process on the host), so `POST /webui/login` exchanges a configured key — through the *same* `subtle.ConstantTimeCompare` comparison and the *same* rate-limited rejection logging the bearer path uses — for a cookie holding 32 `crypto/rand` bytes: `HttpOnly`, `SameSite=Strict`, `Path=/`, and `Secure` when the request arrived over TLS. Server-side sessions are in memory only, expire, and are bounded, so an attacker cannot grow the map. **The bearer path is checked first and is completely ungated by any of this**, so existing OpenAI clients keep working unchanged; only the cookie path additionally requires a same-origin check and a matching `X-Joshbot-CSRF` header (constant-time compared, served per session by `GET /webui/config`) on non-GET requests. The page is served under a `default-src 'none'` CSP with no inline script or style, and model output is rendered as text nodes, never `innerHTML`. Deliberately **not** in v1, each with its own issue: a model picker (the API is agent-as-model — `/v1/models` advertises one id and the `model` field is ignored, so a picker would be a lie, #296), a settings panel (editing config over HTTP is shell-grade privilege escalation, #297) and tool-progress lines in the transcript (`agent.StreamEvent` carries only `{Delta, Done}` and the API never installs `agent.WithSink`, so they do not exist to render — the design mockup's versions were removed rather than faked, #298).

- **`POST /v1/audio/transcriptions` on the HTTP API (#234, transcriptions half).** `joshbot serve` now transcribes a `multipart/form-data` upload with the speech-to-text provider already configured under `stt`, so a client speaking the OpenAI audio dialect reaches the same transcriber Telegram voice notes use, with one credential store rather than two. It is the one route that is not the agent: no ReAct loop, no session, no memory. Without `stt.provider` it answers **501 naming the config key**, because a 404 reads as "joshbot cannot do this" and a 200 with an empty transcript cannot be told apart from silence; a broken `stt` block is fatal at `serve` startup rather than at the first request. The body is read through `r.MultipartReader` rather than `ParseMultipartForm`, which spills past its memory budget to temp files — streaming keeps the audio off disk and denies an authenticated caller a disk-fill primitive. Uploads are capped at 25 MiB (`providers.MaxAudioBytes`, matching what the upstream endpoint enforces, so joshbot refuses locally and names the limit instead of spending the transfer) and read through a `LimitReader` at cap+1 so over-limit is distinguishable from truncated. **Content decides the type**, exactly as it does for images: `providers.SniffAudio` checks the bytes for flac, mp3, mp4/m4a, ogg, wav or webm, because the multipart filename and its declared Content-Type are both written by the caller — a 25 MiB text file named `voice.mp3` would otherwise be uploaded and billed before anything noticed. `model`, `language`, `prompt` and `temperature` are accepted and ignored for drop-in compatibility; `response_format` honours `json` (default) and `text`. A provider failure is a 502 with the upstream text redacted, per the #238 origin split, since an API caller is authenticated but is not the operator. It is one of the two routes that are not the agent (see `/v1/embeddings` below).

- **`POST /v1/embeddings` on the HTTP API (#234, embeddings half).** `joshbot serve` now embeds one or more texts with the provider named by the new `embeddings` config block, reusing that provider's API key and `api_base` so there is no second credential store. Like `/v1/audio/transcriptions` it is not the agent: no ReAct loop, no session, no memory. joshbot does not consume embeddings itself — `memory_search` is lexical — so the route exists to give a client one endpoint for both chat and retrieval. Without `embeddings.provider` it answers **501 naming the config key**, and a broken `embeddings` block is fatal at `serve` startup rather than at the first request. Unlike `stt` it requires **no API key**: ollama is keyless and is the main local case, so demanding one would make the supported configuration impossible to express. `embeddings.model` defaults per provider (`ollama` → `nomic-embed-text`, `openai` → `text-embedding-3-small`); any other provider must set it. `embeddings.timeout` is a `config.Duration`, so `"60s"` and a bare `60` both mean a minute rather than 60 nanoseconds (#240), and a nonzero value under a second is a fatal config error naming the key. **Vectors are placed by the response's `index` field, never by array position:** the OpenAI schema carries an index precisely because the array may come back reordered, and a vector attached to the wrong input is invisible — the caller stores it, retrieves the wrong text, and nothing ever errors; a duplicate, out-of-range, missing or empty-vector index is an error instead. The caps — 128 inputs and 64 KiB per input, on top of the shared 1 MB body limit, which this route is not exempt from — are enforced **before** the provider is dialled, since a limit checked after the work has run has not limited anything. `input` accepts a bare string or an array of strings (both are real SDK traffic), `encoding_format` accepts `float` (default) and `base64` (raw little-endian float32, what `numpy.frombuffer` expects), and `model` is accepted and ignored for drop-in compatibility. `providers.Embed` refuses an empty `APIBase` rather than defaulting one, the same rule `ListModels`, `ProbeCredential` and `TranscribeAudio` enforce, and bounds the upstream error body before it can reach a log line. A provider failure is a 502 with the upstream text redacted, per the #238 origin split.
...
- **A flood on one session key could stall dispatch for every other user (#245).** Because `bus.MaxConcurrentHandlers` is acquired on the dispatcher before the handler is spawned, the per-`channel:senderID` lock's wait is charged to that dispatch slot -- so one user hammering a chat parked its overflow turns on the one held token, each holding a slot until its own `agents.defaults.timeout` fired, and stalled every unrelated user until the queue drained. The session lock now caps concurrency per key: `MaxConcurrentTurnsPerKey` (8) bounds how many turns for one key may be in flight, and `acquire` returns `ErrKeyLockBusy` at once -- no ref, no wait -- for every turn past the cap. `Agent.Process` maps that to an in-band backpressure reply (`agent -m` exits 1, the HTTP API answers 502, because a turn that did not run is a failure, not an answer), and a context cancellation at the gate is reported as a timeout, not a busy rejection. `TestLockSessionRejectsOverflowPerKey` pins the exact admission count.
- **A `configure --provider X` for a registry-backed provider was silently ignored at start-up (#235).** The legacy provider path in `registerProviders` (`cmd/joshbot/main.go`) knew a hard-coded set of bespoke arms, so a config `configure` happily wrote for openai, anthropic, azure, litellm or custom registered **nothing** -- the operator's key did nothing and the fail-fast then blamed the `enabled` flag that was in fact true. There is now one generic loop over `cfg.Providers`, gated on `providers.IsProviderRegistered(name)` and skipped for the six bespoke arms, that registers every registry-backed provider no bespoke arm owns with the full config (extra headers, `extra_body`, per-request `max_tokens`/`temperature`, a resolved base and model). `TestRegisterProvidersRegistersOpenAICentricProviders` asserts all five register through that one loop; a configured name the registry cannot dial still fails loudly via `noProvidersRegisteredError`, as before. Names in that loop are canonicalized (`providers.CanonicalProviderName`) before the skip list, the fallback-order lookup and the `Register` call, because the registry aliases and lowercases them -- a config key of `local`, `nim` or `Ollama` would otherwise slip past the exact-match skip and register a second chain entry for a provider a bespoke arm already owns, without that arm's default base or timeout.
- **`ContextLogger` dropped the `trace_id` when the context already carried one (#290).** The `keyValues = append([…]` was inside the `if !ok || traceID == ""` branch in `internal/log/logger.go`, so a context that already held a trace ID produced a logger with no `trace_id` field at all — the opposite of intent. Every correctly-propagated request logged untraceable; only the requests that *lost* their trace ID got labelled. `WithContext` in the same file did it correctly; the two disagreed. The append now runs unconditionally, so the trace ID is always stamped, whether it comes from the context or is freshly generated. `TestLoggerWithContext` pins the fix by writing through a file-backed logger and asserting the trace ID appears in the output. `TestContextLoggersAttachATraceID` had asserted the buggy behaviour ("ContextLogger must not add a trace ID the context already carried"); that assertion is now inverted to match the corrected logic.
- **`web_fetch` fed raw script/CSS/comment bodies into the model context (#290).** `removeTag` in `internal/tools/web.go` found `<script`, found the next `>`, and deleted only that span — the script body (`var leaked = "SECRET"` etc.), the CSS body, and any comment body stayed in the HTML that was then offered as page content. This was both token burn on every fetch and a prompt-injection surface where attacker-controlled page script text reached the model as ordinary text. The function now takes `(html, openTag, closeTag)` and removes the complete span including the body by finding the matching close tag directly in `rest` from the start of the open tag onward; a dangling open (no close found) drops the remainder, since an unclosed element body is untrusted. This also correctly handles HTML comments (`<!--…-->`) without the prior `>`-skip that broke that case. `TestWebTool_FetchHTMLExtractsTitleAndStripsScripts` now asserts `SCRIPTSECRET` and `STYLESECRET` are absent from the output instead of documenting the leak as a known limitation.
- **The script/style/comment strip was bypassable by changing the tag's case, and the shared `indexFold` mis-indexed on length-changing runes (#290).** `removeTag` matched with `strings.Index`, but tag names are case-insensitive in HTML and the *page* chooses the case: `<SCRIPT>var x="LEAKED";</SCRIPT>` was left whole, so the injection surface the fix above closes stayed open to anyone who pressed shift. Matching now folds ASCII case, pinned by uppercase and mixed-case subtests in `TestWebTool_RemoveTag` that go red the moment it is narrowed back. Folding reuses the existing `indexFold` in `internal/tools/mcp_tool.go`, which was itself unsafe for the one caller that needs an offset: it returned `strings.Index(strings.ToLower(s), …)`, and `unicode.ToLower` can change a string's byte length, so `sanitizeMCPDescription` — which slices the *original* description at that offset to defang joshbot's prompt-envelope tags — would have cut in the wrong place on a description carrying one length-changing rune ahead of a `</skills>`, defanging nothing. It now walks bytes with an ASCII fold, so the offset always indexes the string the caller slices.
...
- **Subagents run a full ReAct loop with tool access** (#194). `internal/subagent` now runs a bounded ReAct loop (model → tools → reflect → repeat) instead of a single chat turn, with process isolation (a fresh message list per run, no session leaks), streaming via a `StreamSink`, and leaf vs orchestrator role scaffolding (no spawn tool is wired up yet, so an orchestrator cannot currently spawn child subagents). Model, max tokens, temperature and timeout are configurable per run. The `parallel_subagent` and `chain_execution` tools use it, and async tools now deliver their real result to the subagent rather than a "started in background" placeholder.
