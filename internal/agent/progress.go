...
const (
	...
	// successfully or not.
	ToolProgressDone
	// ToolProgressNote is emitted zero or more times *during* a tool call, by
	// the tool itself via tools.WithProgress — unlike Start/Done, which
	// bracket the whole call and are generated by the ReAct loop, this is a
	// self-authored checkpoint (e.g. "falling back to Exa MCP") the loop does
	// not and cannot generate on its own, because only the tool knows what it
	// is doing mid-call. A consumer that switches on Phase without handling
	// this case must not treat it as ToolProgressDone — Elapsed and Err are
	// meaningless here (always zero/nil), so rendering it with the "done"
	// format would show a bogus zero-elapsed completion line.
	ToolProgressNote
)
...
type ToolProgressEvent struct {
	...
	Summary string
	// Phase is ToolProgressStart, ToolProgressDone or ToolProgressNote.
	Phase ToolProgressPhase
	// Elapsed is the tool's execution duration. Zero on ToolProgressStart and
	// ToolProgressNote.
	Elapsed time.Duration
	// Err is the tool execution error, if any. Always nil on ToolProgressStart
	// and ToolProgressNote.
	Err error
}
