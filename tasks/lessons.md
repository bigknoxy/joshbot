...
## 2026-08-15 review follow-up (PR #284): docs claimed a behavior the code didn't deliver

- **Failure mode**: CHANGELOG/AGENTS.md claimed persistence "covers client-disconnect cancellation too", but `context.Canceled` fell through to the generic error branch and dropped the session — the doc was written aspirationally ahead of the code. **Detection**: independent review (critical pass + testing/maintainability specialists) probed the actual Canceled path and found saves=0. **Prevention**: when a doc says "covers X", write the regression test that proves X before (or while) writing the sentence; a doc claim that no test enforces is a bug report in waiting. The `TestCancelledTurnPersistsSession` test is pinned to fail on the pre-fix code.
- **Prevention (test quality)**: asserting `saves() > 0` proves a save was attempted, not that the content survived. Assert the stored session carries the user's message.
- **Prevention (async test determinism)**: to test a mid-turn cancellation deterministically, have the provider block on `<-ctx.Done()` and return `ctx.Err()`; the cancel cannot race ahead of the turn because Process cannot return before the provider does.
